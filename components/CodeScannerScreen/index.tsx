...
type CodeScannerScreenNavigationProp = StackNavigationProp<
  RootStackParamList,
  'code-scanner'
>;
...
const CodeScannerScreen: React.FC<CodeScannerScreenProps> = ({
  navigation,
  route,
}) => {
    ...
    async function Start() {
      ...
      if (permition?.granted) {
        ...
        if (perm.granted) {
          ...
          Alert.alert(
            'Erro',
            'Para fazer a leitura do código de barras é necessário permitir o uso da câmera',
          );
          navigation?.navigate(route?.params?.screen as ScreensScanCode, {
            result: false,
          });
        }
      }
  ...
  const codeScannerView = (props: any) => (
    <CameraView onBarcodeScanned={onScanBarCode} {...props} />
  );

  return <Box w="$full" h="$full" as={codeScannerView}></Box>;
};
