...
const View: React.FC<ListarProdutosScreenProps> = ({
    navigation,
    route,
  }) => {
    ...
    const [products, setProdutcts] = React.useState<Array<UpdateProdutoDto | unknown>>([]);
    ...
    const getProdutos = async (type: "all" | "nome" | "codigo_de_barras" | "categoria" | "marca" | "tipo" | "empresa") => {
        switch(type){
            case "all":
            ...
            case "codigo_de_barras":
                ...
                break;
            case "categoria":
                const produtosCategoria = await new Produto(db).findAllByCategory(Number(paramSearch));
                setHaveProducts(produtosCategoria.length > 0);
                setProdutcts(produtosCategoria);
                break;
            case "empresa":
                const produtosEmpresa = await new Produto(db).findAllByIdEmpresa(Number(paramSearch));
                setHaveProducts(produtosEmpresa.length > 0);
                setProdutcts(produtosEmpresa);
                break;
            case "marca":
                const produtosMarca = await new Produto(db).findFirstByIdMarca(Number(paramSearch));
          ...
          <Box w="$full" h="$full" alignItems="center" justifyContent="center">
            <Box gap={10}>
              ...
              <Button
                ...
                gap={10}
                onPress={() => navigation?.navigate(temEmpresas ? 'cadastrar-produtos' : 'cadastrar-empresas')}
              >
                <ButtonText>{temEmpresas ? "Cadastrar Produtos" : "Cadastrar Empresas"}</ButtonText>
                <ButtonIcon
                  color="$white"
                  as={() => (
