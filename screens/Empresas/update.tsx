...
const validator = Yup.object().shape({
    ...
    emails: Yup.array().of(
        Yup.object().shape({
        id: Yup.number().required(ValidateForms.required),
        email: Yup.string()
            .email(ValidateForms.email)
            .required(ValidateForms.required),
        id_empresa: Yup.number().required(ValidateForms.required),
        }),
    ),
    telefones: Yup.array()
    .of(
...
const Update: React.FC<EditarEmpresasScreenProps> = ({ navigation, route }) => {
  if (!route) {
    ...
    const [empresam, setEmpresa] = React.useState({});
    ...
    const obj_empresa = route.params.empresa;
    const start =  async () => {
        const endereco = await new Endereco(db).findById(obj_empresa.id);
        const emails = await new Email(db).findAllByIdEmpresa(obj_empresa.id);
        const telefones = await new Telefone(db).findByIdEmpresa(obj_empresa.id);
        setEmpresa({
            ...obj_empresa,
            ...endereco,
            emails,
    ...
    <ScrollView>
      <Box mx="$10" my="$5" gap="$5">
        ...
        <Formik
          ...
          initialValues={{
            ...empresa as ,
            tipo_empresa: empresa.nome_completo ? 'pf' : 'pj' as  'pf' | 'pj',
          }}
