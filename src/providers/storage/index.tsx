...
export const StorageProvider: React.FC<StorageProviderProps> = ({
  ...
  const getMultiItems = (keys: string[]) =>
    keys.reduce((acc, key) => ({ ...acc, [key]: state[key] }), {});
  const setItem = (key: string, value: any) =>
    dispatch({ type: 'SET_ITEM', key, value });
  const setMultiItems = (items: { [key: string]: any }) =>
    dispatch({ type: 'SET_MULTI_ITEMS', items });
  const clearItem = (key: string) => dispatch({ type: 'REMOVE_ITEM', key });
  const clearMultiItems = (keys: string[]) =>
    dispatch({ type: 'REMOVE_MULTI_ITEMS', keys });
