...
CREATE TABLE cliente (
    ...
    limite DECIMAL(10, 2),
    e_empresa INTEGER NOT NULL CHECK (e_empresa IN (0, 1)) -- Booleano (0 ou 1)
);
...
CREATE TABLE pessoa (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    nome TEXT NOT NULL,
    cpf TEXT UNIQUE -- CPF com restrição de unicidade
);
...
CREATE INDEX idx_pessoa_cliente_id_endereco ON pessoa_cliente(id_endereco);
...
CREATE TABLE pessoa_cliente_telefone (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    id_pessoa_cliente INTEGER NOT NULL,
    id_telefone INTEGER NOT NULL,
    FOREIGN KEY (id_pessoa_cliente) REFERENCES pessoa_cliente(id),
    FOREIGN KEY (id_telefone) REFERENCES telefone(id)
);

-- Índices nas chaves estrangeiras da tabela pessoa_cliente_telefone
CREATE INDEX idx_pessoa_cliente_telefone_id_pessoa_cliente ON pessoa_cliente_telefone(id_pessoa_cliente);
CREATE INDEX idx_pessoa_cliente_telefone_id_telefone ON pessoa_cliente_telefone(id_telefone);
...
-- Tabela pessoa_cliente_email
CREATE TABLE pessoa_cliente_email (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    id_pessoa_cliente INTEGER NOT NULL,
...
CREATE INDEX idx_pessoa_cliente_email_id_pessoa_cliente ON pessoa_cliente_email(id_pessoa_cliente);
CREATE INDEX idx_pessoa_cliente_email_id_email ON pessoa_cliente_email(id_email);
...
CREATE INDEX idx_empresa_endereco_id_endereco ON empresa_endereco(id_endereco);
